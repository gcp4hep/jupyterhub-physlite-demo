...
def get_signed_url_worker(proxy_data, scope, name, rucio_account="nihartma", rucio_home='/srv/conda/envs/notebook', ca_cert=None):
    import rucio.client
    setup_rucio_and_proxy(proxy_data, rucio_account=rucio_account, rucio_home=rucio_home, proxy_path="x509")
    rucio_client = rucio.client.Client(ca_cert=ca_cert)
